...
export default function ReviewPage() {
  ...
  useEffect(() => {
    async function load() {
      if (!token) return
      const { data: l } = await supabase.from('coord_review_links').select('*').eq('token', token).maybeSingle()
      if (!l) { setNotFound(true); setLoading(false); return }
      setLink(l)
      const { data: rs } = await supabase.from('coord_review_responses').select('*').eq('token', token)
      const map = {}
      const its = l.items || []
      ;(rs || []).forEach(r => {
        let idx = -1
  ...
  async function setDecision(idx, decision) {
    ...
    setSavingIdx(idx)
    const { error } = await supabase.from('coord_review_responses').upsert(
