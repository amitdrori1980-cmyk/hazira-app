'use client'
// HAZIRA-PLANSHARE-V1
import { useEffect, useState } from 'react'
import { supabase } from '@/lib/supabase'

const HE_DAYS   = ['א׳','ב׳','ג׳','ד׳','ה׳','ו׳','ש׳']
const HE_MONTHS = ['ינואר','פברואר','מרץ','אפריל','מאי','יוני','יולי','אוגוסט','ספטמבר','אוקטובר','נובמבר','דצמבר']
function fmtDay(ds){ if(!ds) return 'ללא תאריך'; const [y,m,d]=String(ds).split('-').map(Number); if(!y||!m||!d) return 'ללא תאריך'; const dt=new Date(y,m-1,d); return 'יום '+HE_DAYS[dt.getDay()]+' · '+d+' '+HE_MONTHS[m-1] }
function fmtShort(ds){ if(!ds) return ''; const [y,m,d]=String(ds).split('-').map(Number); if(!y||!m||!d) return ''; return d+'/'+m }
function sortColsByDate(arr){ return [...arr].sort((a,b)=>String(a.date||'9999-12-31').localeCompare(String(b.date||'9999-12-31'))) }

const PLAN_STATUSES = [
  { value:'draft', label:'טיוטה' },
  { value:'active', label:'פעיל' },
  { value:'done', label:'הושלם' },
]
const getPlanStatus = v => PLAN_STATUSES.find(s=>s.value===v) || PLAN_STATUSES[0]

const DAY_CATEGORIES = [
  { value:'',        label:'',       head:'#B6CFD0', text:'#33414A' },
  { value:'prep',    label:'הכנות',  head:'#D8C7A8', text:'#5E4E30' },
  { value:'rehears', label:'חזרות',  head:'#AEC3D0', text:'#33495A' },
  { value:'show',    label:'מופע',   head:'#D3A9BE', text:'#6E2E4E' },
  { value:'strike',  label:'פירוק',  head:'#C3C6C9', text:'#40454A' },
]
const getDayCategory = v => DAY_CATEGORIES.find(c=>c.value===(v||'')) || DAY_CATEGORIES[0]

const HAZIRA_LOGO = 'data:image/svg+xml;base64,'

export default function SharedPlanPage({ params }) {
  const id = params?.id
  const [loading, setLoading] = useState(true)
  const [plan, setPlan] = useState(null)
  const [days, setDays] = useState([])

  useEffect(() => { if (id) load() }, [id])

  async function load() {
    setLoading(true)
    const { data: p } = await supabase.from('project_plans').select('*').eq('id', id).single()
    if (!p) { setPlan(null); setLoading(false); return }
    const [{ data: cols }, { data: cells }] = await Promise.all([
      supabase.from('project_plan_columns').select('*').eq('plan_id', id).order('sort_order'),
      supabase.from('project_plan_cells').select('*').eq('plan_id', id).order('sort_order'),
    ])
    const grouped = {}
    ;(cells || []).forEach(c => { if (!grouped[c.column_id]) grouped[c.column_id] = []; grouped[c.column_id].push(c) })
    setPlan(p)
    setDays(sortColsByDate(cols || []).map(col => ({ ...col, cells: grouped[col.id] || [] })))
    setLoading(false)
  }

  if (loading) return <div dir="rtl" className="min-h-screen flex items-center justify-center text-gray-400">טוען...</div>

  if (!plan) return (
    <div dir="rtl" className="min-h-screen flex flex-col items-center justify-center text-center px-6 bg-gray-50">
      <img src={HAZIRA_LOGO} alt="הזירה" className="h-16 mb-4 opacity-80" />
      <div className="text-gray-700 font-semibold text-lg">התוכנית לא נמצאה</div>
      <div className="text-gray-400 text-sm mt-1">ייתכן שהקישור שגוי או שהתוכנית נמחקה</div>
    </div>
  )

  const dates = days.map(d => d.date).filter(Boolean)
  const range = dates.length ? (fmtShort(dates[0]) + (dates.length > 1 ? ' – ' + fmtShort(dates[dates.length - 1]) : '')) : ''
  const st = getPlanStatus(plan.status)

  return (
    <div dir="rtl" className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-2xl mx-auto">
        <header className="flex items-center justify-between gap-4 border-b-2 border-[#E0197D] pb-4 mb-5">
          <div className="min-w-0">
            <div className="text-[11px] font-semibold text-[#E0197D] tracking-wide mb-0.5">הזירה · תכנון פרויקטים</div>
            <h1 className="text-2xl font-bold text-gray-900 leading-tight break-words">{plan.title}</h1>
            <div className="text-[13px] text-gray-500 mt-1">{st.label}{range ? ' · ' + range : ''}{days.length ? ' · ' + days.length + ' ימים' : ''}</div>
          </div>
          <img src={HAZIRA_LOGO} alt="הזירה" className="h-14 w-auto flex-shrink-0" />
        </header>

        {plan.notes && (
          <div className="bg-white border border-gray-100 rounded-xl p-4 mb-5 text-[13px] text-gray-700 whitespace-pre-wrap break-words">
            <div className="text-[11px] text-gray-400 mb-1">הערות כלליות</div>
            {plan.notes}
          </div>
        )}

        <div className="space-y-4">
          {days.map(day => {
            const cat = getDayCategory(day.category)
            return (
              <section key={day.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                <div className="flex items-center justify-between px-4 py-2.5" style={{ backgroundColor: cat.head, color: cat.text }}>
                  <span className="font-bold text-sm">{fmtDay(day.date)}</span>
                  {cat.value && <span className="text-[11px] font-semibold border rounded-full px-2.5 py-0.5" style={{ borderColor: cat.text, background: 'rgba(255,255,255,.35)' }}>{cat.label}</span>}
                </div>
                <div className="p-3 space-y-2">
                  {day.cells.length ? day.cells.map(c => (
                    <div key={c.id} className="border border-gray-100 rounded-lg p-3">
                      <div className="text-[13px] font-semibold text-gray-900 whitespace-pre-wrap break-words">{c.action || '—'}</div>
                      {c.crew && <div className="text-[12px] text-gray-600 mt-1 whitespace-pre-wrap break-words">{c.crew}</div>}
                      {c.notes && <div className="text-[12px] text-gray-400 mt-1 whitespace-pre-wrap break-words">{c.notes}</div>}
                    </div>
                  )) : <div className="text-center text-[12px] text-gray-300 py-3">אין פעולות</div>}
                </div>
              </section>
            )
          })}
          {days.length === 0 && <div className="text-center text-gray-400 py-10">אין ימים בתוכנית</div>}
        </div>

        <footer className="text-center text-[11px] text-gray-400 mt-8 pt-4 border-t border-gray-200">הזירה · מערכת ניהול הפקה</footer>
      </div>
    </div>
  )
}
