        ...
        <div className="bg-white border border-gray-100 rounded-xl p-4 mb-4">
          ...
          <div className="grid grid-cols-2 gap-2 mb-3">
            <input value={newEvent.event_name} onChange={e=>setNewEvent(p=>({...p,event_name:e.target.value}))}
              placeholder="שם האירוע *" className="text-sm px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#FF3EB5] text-right col-span-2"/>
            ...
            <select value={newEvent.venue} onChange={e=>setNewEvent(p=>({...p,venue:e.target.value}))}
              className="text-sm px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#FF3EB5] col-span-2">
            ...
            <div className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-gray-50 flex-row-reverse"
                ...
                {editingEvent === ev.id ? (
                  <div className="flex gap-2 flex-row-reverse" onClick={e=>e.stopPropagation()}>
                    <input value={editEventVal.event_name||''} onChange={e=>setEditEventVal(p=>({...p,event_name:e.target.value}))}
                      className="text-sm font-medium px-2 py-1 border border-[#FF3EB5] rounded-lg outline-none text-right flex-1"/>
                    ...
                    <button onClick={saveEventEdit} className="text-[#FF3EB5] text-sm font-medium">שמור</button>
              ...
              <div className="flex items-center gap-1">
                <button onClick={e=>{e.stopPropagation();setCollapsedEvents(p=>({...p,[ev.id]:!p[ev.id]}))}}
                  className="text-gray-300 hover:text-[#FF3EB5] p-1" title={collapsedEvents[ev.id]?'הרחב':'כווץ'}>
                ...
                <button onClick={e=>{e.stopPropagation();pushToCalendar(ev)}}
                  className="text-gray-300 hover:text-[#FF3EB5] p-1" title="עדכן ביומן">
  ...
  return (
    <div className="mb-4 no-print">
      <button onClick={handleOpen}
        className={`w-full flex items-center gap-2 px-4 py-2.5 rounded-xl border transition-colors flex-row-reverse text-right text-[13px] ${open ? 'bg-[#FFE6F5] border-[#FF3EB5] text-[#FF3EB5]' : 'bg-white border-gray-100 text-gray-600 hover:border-[#FF3EB5]'}`}>
        <i className="ti ti-folder text-[#FF3EB5]" style={{fontSize:15}}/>
        ...
        <div className="mt-1 bg-white border border-gray-100 rounded-xl overflow-hidden">
          ...
          {files.map(f => (
            <div key={f.name} className="flex items-center gap-3 px-4 py-3 border-b border-gray-50 last:border-0 hover:bg-gray-50 flex-row-reverse group">
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${isExcel(f.name) ? 'bg-green-50' : 'bg-[#FFE6F5]'}`}>
                <i className={`ti ${isExcel(f.name) ? 'ti-file-spreadsheet text-green-600' : 'ti-file-type-pdf text-[#FF3EB5]'}`} style={{fontSize:16}}/>
              </div>
              ...
              {isExcel(f.name) ? (
                ...
                <button onClick={() => openFile(f.name)}
                  className="text-[12px] text-[#FF3EB5] border border-[#FF3EB5] px-2 py-1 rounded-lg flex items-center gap-1 flex-shrink-0 md:opacity-0 md:group-hover:opacity-100 md:transition-opacity">
      ...
      <div className="max-w-4xl">
        ...
        {generalFileViewer && (
          <div className="fixed inset-0 z-50 bg-black/70 flex flex-col no-print">
            <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-gray-100 flex-row-reverse">
              ...
              <a href={generalFileViewer.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[13px] text-[#FF3EB5] hover:underline">
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-5 bg-gray-50 md:hidden px-6 text-center">
              <a href={generalFileViewer.url} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-2 text-[14px] bg-[#FF3EB5] text-white px-6 py-3 rounded-xl font-medium">
        ...
        {selectedEvent && !loading && !schedule && isManager && (
          <div className="bg-white border border-gray-100 rounded-xl p-8 text-center no-print">
            ...
            <button onClick={createSchedule} className="bg-[#FF3EB5] text-white px-6 py-2.5 rounded-lg text-sm hover:bg-[#CC0090]">+ צור לוז חדש</button>
          </div>
        )}
        {selectedEvent && !loading && !schedule && !isManager && (
          ...
          <>
            {isManager && (
              <div className="bg-white border border-gray-100 rounded-xl p-3 mb-4 flex items-center gap-3 flex-wrap no-print">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <button onClick={() => updateSchedule('visible_to', 'managers')}
                      className={`text-[11px] px-2.5 py-1 rounded-full border transition-colors ${schedule.visible_to==='managers'?'bg-[#FF3EB5] text-white border-[#FF3EB5]':'border-gray-200 text-gray-500 hover:border-[#FF3EB5]'}`}>
                      מנהלים בלבד
                    </button>
                    <button onClick={() => updateSchedule('visible_to', 'all')}
                      className={`text-[11px] px-2.5 py-1 rounded-full border transition-colors ${schedule.visible_to==='all'?'bg-[#FF3EB5] text-white border-[#FF3EB5]':'border-gray-200 text-gray-500 hover:border-[#FF3EB5]'}`}>
                      כולם
                    </button>
                    <button onClick={() => updateSchedule('visible_to', 'specific')}
                      className={`text-[11px] px-2.5 py-1 rounded-full border transition-colors ${schedule.visible_to==='specific'?'bg-[#FF3EB5] text-white border-[#FF3EB5]':'border-gray-200 text-gray-500 hover:border-[#FF3EB5]'}`}>
                      אנשים ספציפיים
                    </button>
                  </div>
                </div>
                      ...
                      {crew.map(c => {
                        ...
                        return (
                          <button key={c.id}
                            ...
                            className={`text-[11px] px-2.5 py-1 rounded-full border transition-colors ${selected?'bg-[#E1F5EE] text-[#085041] border-[#085041]':'border-gray-200 text-gray-500 hover:border-[#FF3EB5]'}`}>
                            {c.full_name}
                          </button>
                        )
                      })}
                ...
                <button onClick={exportExcel} disabled={exporting}
                  className="flex items-center gap-1.5 text-[12px] px-3 py-1.5 border border-gray-200 rounded-lg hover:border-[#FF3EB5] text-gray-600 disabled:opacity-50">
                  <i className="ti ti-file-spreadsheet" style={{fontSize:14}}/> אקסל
                </button>
                <button onClick={() => window.print()}
                  className="flex items-center gap-1.5 text-[12px] px-3 py-1.5 border border-gray-200 rounded-lg hover:border-[#FF3EB5] text-gray-600">
            ...
            {!isManager && schedule.status==='final' && (
              <div className="flex gap-2 mb-4 justify-end no-print">
                <button onClick={exportExcel} disabled={exporting} className="flex items-center gap-1.5 text-[12px] px-3 py-1.5 border border-gray-200 rounded-lg hover:border-[#FF3EB5] text-gray-600 bg-white">
                  <i className="ti ti-file-spreadsheet" style={{fontSize:14}}/> ייצוא לאקסל
                </button>
                <button onClick={() => window.print()} className="flex items-center gap-1.5 text-[12px] px-3 py-1.5 border border-gray-200 rounded-lg hover:border-[#FF3EB5] text-gray-600 bg-white">
            ...
            <div id="schedule-print" className="bg-white border border-gray-100 rounded-xl overflow-hidden">
              ...
              <div className="px-6 py-5 border-b border-gray-100" style={{borderRight:'4px solid #FF3EB5'}}>
                  ...
                  {isManager ? (
                    <div className="flex items-center gap-2 flex-row-reverse">
                      ...
                      <input value={schedule.participants||''} onChange={e => setSchedule(prev => ({...prev, participants: e.target.value}))}
                        ...
                        className="flex-1 text-[13px] px-3 py-1.5 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#FF3EB5] text-right no-print"/>
              ...
              <div className={`grid gap-0 bg-[#FF3EB5] text-white text-[12px] font-semibold no-print ${isManager?'grid-cols-[80px_1fr_1fr_1fr_40px]':'grid-cols-[80px_1fr_1fr_1fr]'}`}>
              ...
              {rows.map((row, index) => (
                <div key={row.id}
                  ...
                  {isManager ? (
                    ...
                    <>
                      <div className="px-3 py-2.5 text-[13px] text-right border-l border-gray-100 font-mono text-[#FF3EB5] font-medium">{row.time}</div>
              ...
              {isManager && (
                <button onClick={addRow}
                  className="w-full py-3 text-[13px] text-gray-400 hover:text-[#FF3EB5] hover:bg-[#FFE6F5] transition-colors flex items-center justify-center gap-1 no-print">
      ...
      <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
        ...
        {files.map(f => (
          <div key={f.name} className="flex items-center gap-2 px-4 py-3 border-b border-gray-50 last:border-0 group hover:bg-gray-50 flex-row-reverse">
            ...
            <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${isExcel(f.name) ? 'bg-green-50' : 'bg-[#FFE6F5]'}`}>
              <i className={`ti ${isExcel(f.name) ? 'ti-file-spreadsheet text-green-600' : 'ti-file-type-pdf text-[#FF3EB5]'}`} style={{fontSize:18}}/>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center px-4 pb-6 md:pb-0" style={{background:'rgba(0,0,0,0.4)'}}>
          <div className="bg-white rounded-2xl w-full max-w-sm p-5 shadow-xl">
            <div className="flex items-center justify-center w-12 h-12 bg-red-50 rounded-full mx-auto mb-3">
              <i className="ti ti-trash text-[#FF3EB5]" style={{fontSize:22}}/>
            </div>
            ...
            <div className="flex gap-2">
              ...
              }} className="flex-1 py-2.5 rounded-xl bg-[#FF3EB5] text-white text-[14px]">מחק</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-2 mb-4">
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`text-[13px] px-4 py-2 rounded-lg border transition-colors ${tab===t.id?'bg-[#FF3EB5] text-white border-[#FF3EB5]':'border-gray-200 text-gray-600 hover:border-[#FF3EB5]'}`}>
            {t.label}
          </button>
        ))}
      </div>
