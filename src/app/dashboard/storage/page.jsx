      ...
      <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
        ...
        {loading ? (
          ...
          <div key={item.id} className="border-b border-gray-50 last:border-0">
            {editing === item.id ? (
              <div className="p-3 flex flex-col gap-2 bg-[#FDEAEA]">
                <input value={editVal.name} onChange={e=>setEditVal(v=>({...v,name:e.target.value}))}
                  placeholder="פריט"
                  className="text-sm px-3 py-1.5 border border-gray-200 rounded-lg bg-white outline-none focus:border-[#CC1010]"/>
                ...
                <div className="flex gap-2">
                  <button onClick={() => saveEdit(item.id)} className="flex-1 bg-[#CC1010] text-white text-sm py-1.5 rounded-lg">שמור</button>
              ...
              <div className="grid grid-cols-12 gap-2 px-4 py-2.5 items-center group hover:bg-gray-50">
                ...
                <div className="col-span-4 text-[12px] text-gray-500 text-right flex items-center gap-1 justify-end">
                  {item.location && <><i className="ti ti-map-pin text-[#CC1010]" style={{fontSize:10}}/>{item.location}</>}
                </div>
