    ...
    <div className="max-w-xl">
          ...
          <form onSubmit={addMember} className="flex flex-col gap-2">
            <div className="grid grid-cols-2 gap-2">
              <input value={form.full_name} onChange={e=>setForm(f=>({...f,full_name:e.target.value}))}
                ...
                className="col-span-2 text-sm px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#E0197D]"/>
            ...
            <button type="submit" disabled={adding}
              className="bg-[#E0197D] text-white text-sm py-2 rounded-lg hover:bg-[#A0106A] transition-colors disabled:opacity-50">
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[11px] bg-[#FCE4F3] text-[#A0106A] px-2 py-0.5 rounded-full">{filtered.length}</span>
        </div>
          ...
          <div key={member.id} className="border-b border-gray-50 last:border-0">
            {editing === member.id ? (
              <div className="py-3 flex flex-col gap-2">
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <input value={editVal.email||''} onChange={e=>setEditVal(v=>({...v,email:e.target.value}))}
                    placeholder="אימייל" className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#E0197D]"/>
                </div>
                <input value={editVal.notes||''} onChange={e=>setEditVal(v=>({...v,notes:e.target.value}))}
                  placeholder="הערות" className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#E0197D]"/>
                <div className="flex gap-2">
                  <button onClick={() => saveEdit(member.id)} className="flex-1 bg-[#E0197D] text-white text-sm py-1.5 rounded-lg">שמור</button>
              ...
              <div className="flex items-center gap-3 py-2.5 flex-row-reverse group">
                <div className="w-8 h-8 rounded-full bg-[#FCE4F3] text-[#E0197D] text-[11px] font-semibold flex items-center justify-center flex-shrink-0">
