...
export default function GearPage() {
  ...
  const tabBtn = (id, label) => (
    <button onClick={() => setTab(id)}
      className={`text-[13px] px-4 py-1.5 rounded-lg font-medium transition-colors ${tab === id ? 'bg-[#E0197D] text-white' : 'text-gray-500 hover:text-[#E0197D]'}`}>
      {label}
    </button>
  )
  return (
    <div>
      ...
      {tab === 'japan' && <BoardContent label="יפן" rowsTable="japan_rows" cellsTable="japan_cells" />}
      {tab === 'aya' && <BoardContent label="איה" rowsTable="aya_rows" cellsTable="aya_cells" />}
      {tab === 'napa' && <BoardContent label="נאפה" rowsTable="napa_rows" cellsTable="napa_cells" />}
    </div>
  )
}

function BoardContent({ label, rowsTable, cellsTable }) {
  ...
  async function load() {
    setLoading(true)
    const { data, error } = await supabase.from(rowsTable).select('*').order('position', { ascending: true })
    if (error) alert('שגיאה בטעינה: ' + error.message)
    setRows(data || [])
    const { data: meta } = await supabase.from(cellsTable).select('*')
    const m = {}
  ...
  async function addRow(cell) {
    ...
    const { data, error } = await supabase.from(rowsTable).insert({ cell, content: '', position: pos }).select().single()
  ...
  async function deleteRow(id) {
    ...
    await supabase.from(rowsTable).delete().eq('id', id)
  ...
  async function saveRow(id, content) {
    ...
    const row = rows.find(r => r.id === id)
    await supabase.from(rowsTable).update({ content }).eq('id', id)
    if (row) touchCell(row.cell)
  }
  ...
  async function touchCell(cell) {
    ...
    await supabase.from(cellsTable).upsert({ cell, updated_at: now }, { onConflict: 'cell' })
  }
    ...
    return (
      <div className="max-w-2xl">
        <div className="flex items-center justify-between mb-4 flex-row-reverse">
          ...
          <div className="text-[15px] font-semibold text-gray-800">{label} {openCell}</div>
        </div>
      ...
      {CELLS.map(cell => {
        ...
        return (
          <button key={cell} onClick={() => setOpenCell(cell)}
            ...
            <div className="text-[13px] md:text-[16px] font-semibold text-gray-800">{label} {cell}</div>
