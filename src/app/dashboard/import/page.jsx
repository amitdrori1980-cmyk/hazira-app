    ...
    <div className="max-w-2xl">
      ...
      {step === 1 && (
        <div className="bg-white border border-gray-100 rounded-xl p-6">
          ...
          {eventTypes.length > 0 && (
            <div className="bg-[#FCE4F3] rounded-lg p-3 mb-5 text-[12px] text-[#A0106A]">
              <strong>סוגי אירוע מוכרים:</strong>{' '}
              {eventTypes.map(t => t.label).join(' / ')}
            </div>
          )}

          <label className="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-[#E0197D] hover:bg-[#FCE4F3] transition-colors">
            <i className="ti ti-file-spreadsheet text-[#E0197D]" style={{fontSize:36}}/>
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-4 mb-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={()=>{setStep(1);setRows([]);setPreview([])}}
                className="text-[12px] text-gray-400 hover:text-[#E0197D]">
                החלף קובץ
              </button>
            </div>
      ...
      {step === 3 && result && (
        <div className="bg-white border border-gray-100 rounded-xl p-8 text-center">
          ...
          <div className="text-[13px] text-gray-500 mb-6">
            ...
            {result.failed > 0  && <div className="text-[#E0197D]">❌ {result.failed} אירועים נכשלו</div>}
          </div>
          <div className="flex gap-3 justify-center">
            ...
            <a href="/dashboard/calendar"
              className="px-5 py-2 bg-[#E0197D] text-white rounded-lg text-sm hover:bg-[#A0106A]">
              עבור ליומן
            </a>
          </div>
        </div>
