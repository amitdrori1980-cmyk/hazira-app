      ...
      {isManager && (
        <div className="flex justify-end gap-2 mb-4">
          <button onClick={() => fileInputRef.current.click()} disabled={importing}
            className="flex items-center gap-1.5 text-sm px-4 py-2 rounded-lg border border-gray-200 text-gray-600 hover:border-[#1ABBB4] bg-white disabled:opacity-50">
      ...
      {showNewProd && (
        <div className="bg-white border border-gray-100 rounded-xl p-4 mb-4">
          ...
          <input value={newProdName} onChange={e => setNewProdName(e.target.value)}
            ...
            className="w-full text-sm px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#1ABBB4] text-right mb-3"/>
                ...
                {editingProd === prod.id ? (
                  <div className="flex gap-2 flex-row-reverse" onClick={e => e.stopPropagation()}>
                    <input value={editProdName} onChange={e => setEditProdName(e.target.value)}
                      ...
                      className="flex-1 text-[15px] font-semibold px-2 py-1 border border-[#1ABBB4] rounded-lg outline-none text-right"/>
                    <button onClick={() => saveEditProd(prod.id)} className="text-[#1ABBB4] text-sm font-medium">שמור</button>
                ...
                {prodContacts.filter(c => c.email).length > 0 && (
                  <div className="flex items-center gap-2 px-4 py-2.5 bg-gray-50 border-b border-gray-100 flex-row-reverse">
                    <button onClick={() => selectAll(prodContacts)}
                      className="text-[11px] text-gray-500 hover:text-[#1ABBB4] transition-colors">
                      ...
                      <div className="flex-1" onClick={e => e.stopPropagation()}>
                        <div className="grid grid-cols-2 gap-2 mb-2">
                          <input value={editContact.full_name||''} onChange={e => setEditContact(p=>({...p,full_name:e.target.value}))}
                            placeholder="שם מלא" className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg outline-none focus:border-[#1ABBB4] text-right"/>
                          <input value={editContact.role||''} onChange={e => setEditContact(p=>({...p,role:e.target.value}))}
                            placeholder="תפקיד" className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg outline-none focus:border-[#1ABBB4] text-right"/>
                          <input value={editContact.phone||''} onChange={e => setEditContact(p=>({...p,phone:e.target.value}))}
                            placeholder="טלפון" className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg outline-none focus:border-[#1ABBB4] text-right" dir="ltr"/>
                          <input value={editContact.email||''} onChange={e => setEditContact(p=>({...p,email:e.target.value}))}
                            placeholder="אימייל" className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg outline-none focus:border-[#1ABBB4] text-right" dir="ltr"/>
                        </div>
                        <div className="flex gap-2">
                          <button onClick={() => saveEditContact(c.id, prod.id)}
                            className="flex-1 bg-[#1ABBB4] text-white text-sm py-1.5 rounded-lg hover:bg-[#0D7A76]">שמור</button>
                      ...
                      <>
                        ...
                        <div className="flex-1 text-right">
                          ...
                          <div className="flex gap-3 justify-end mt-1 flex-wrap">
                            {c.phone && (
                              <a href={`tel:${c.phone}`} className="flex items-center gap-1 text-[12px] text-[#1ABBB4] hover:underline" dir="ltr">
                ...
                {addingContact === prod.id ? (
                  <div className="px-4 py-3 border-t border-gray-50 bg-gray-50">
                    <div className="grid grid-cols-2 gap-2 mb-2">
                      <input value={newContact.full_name} onChange={e => setNewContact(p=>({...p,full_name:e.target.value}))}
                        placeholder="שם מלא *" className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg outline-none focus:border-[#1ABBB4] text-right bg-white"/>
                      <input value={newContact.role} onChange={e => setNewContact(p=>({...p,role:e.target.value}))}
                        placeholder="תפקיד" className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg outline-none focus:border-[#1ABBB4] text-right bg-white"/>
                      <input value={newContact.phone} onChange={e => setNewContact(p=>({...p,phone:e.target.value}))}
                        placeholder="טלפון" className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg outline-none focus:border-[#1ABBB4] bg-white" dir="ltr"/>
                      <input value={newContact.email} onChange={e => setNewContact(p=>({...p,email:e.target.value}))}
                        placeholder="אימייל" className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg outline-none focus:border-[#1ABBB4] bg-white" dir="ltr"/>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => addContact(prod.id)} disabled={savingContact}
                        className="flex-1 bg-[#1ABBB4] text-white text-sm py-1.5 rounded-lg hover:bg-[#0D7A76] disabled:opacity-50">
                  ...
                  <div className="flex border-t border-gray-50">
                    <button onClick={() => setAddingContact(prod.id)}
                      className="flex-1 py-3 text-[13px] text-gray-400 hover:text-[#1ABBB4] hover:bg-[#E6F7F7] transition-colors flex items-center justify-center gap-1">
