    ...
    <div className="max-w-xl">
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-4 mb-4">
        ...
        <form onSubmit={addMember} className="grid grid-cols-2 gap-2">
          <input value={form.full_name} onChange={e=>setForm(f=>({...f,full_name:e.target.value}))} placeholder="שם מלא *" className="col-span-2 text-sm px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#1ABBB4]" />
          <input value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} placeholder="אימייל *" type="email" className="text-sm px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#1ABBB4]" />
          <input value={form.password} onChange={e=>setForm(f=>({...f,password:e.target.value}))} placeholder="סיסמה *" className="text-sm px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#1ABBB4]" />
          <input value={form.role} onChange={e=>setForm(f=>({...f,role:e.target.value}))} placeholder="תפקיד" className="text-sm px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#1ABBB4]" />
          ...
          <label className="col-span-2 flex items-center gap-2 text-sm text-gray-600 cursor-pointer flex-row-reverse justify-end">
            <input type="checkbox" checked={form.is_manager} onChange={e=>setForm(f=>({...f,is_manager:e.target.checked}))} style={{ accentColor:'#1ABBB4' }} />
            מנהל הפקה (גישה מלאה)
          </label>
          <button type="submit" disabled={adding} className="col-span-2 bg-[#1ABBB4] text-white text-sm py-2 rounded-lg hover:bg-[#0D7A76] transition-colors">
            קבל הוראות הוספה
          </button>
        </form>
        {msg && (
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-4">
        ...
        {loading ? (
          ...
          <div key={m.id} className="flex items-center gap-3 py-2.5 border-b border-gray-50 last:border-0 flex-row-reverse">
            <div className="w-8 h-8 rounded-full bg-[#E6F7F7] text-[#1ABBB4] text-[11px] font-medium flex items-center justify-center flex-shrink-0">
            ...
            <span className="text-[11px] bg-[#E6F7F7] text-[#0D7A76] px-2 py-0.5 rounded-full">{m.dept}</span>
