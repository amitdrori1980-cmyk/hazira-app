...
export default function EquipmentContent() {
  ...
  function ParamFields({ paramType, val, setVal, compact=false }) {
    const cls = `text-sm px-2 py-1.5 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#E0197D] ${compact ? 'text-[12px]' : ''}`
    return (
          ...
          <div key={cat.id} className="mb-3">
            ...
            <button onClick={() => toggleCat(cat)}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all flex-row-reverse text-right ${
                openCat === cat.id
                  ? 'bg-[#E0197D] text-white border-[#E0197D]'
                  : 'bg-white border-gray-100 hover:border-[#E0197D] text-gray-800'
              }`}>
            ...
            {/* Subcategories */}
            {openCat === cat.id && (
              <div className="mt-1 mr-3 border-r-2 border-[#E0197D] pr-3">
                  ...
                  <div key={sub.id} className="mb-2">
                    ...
                    <button onClick={() => toggleSub(sub)}
                      className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg border transition-all flex-row-reverse text-right ${
                        openSub === sub.id
                          ? 'bg-[#FCE4F3] border-[#E0197D] text-[#E0197D]'
                    ...
                    {openSub === sub.id && (
                      <div className="mt-1 bg-white border border-gray-100 rounded-lg overflow-hidden">
                            ...
                            {editing === item.id ? (
                              <div className="p-2 flex flex-col gap-1.5">
                                <input value={editVal.name} onChange={e=>setEditVal(v=>({...v,name:e.target.value}))}
                                  className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#E0197D]" dir="ltr"/>
                                <ParamFields paramType={sub.param_type} val={editVal} setVal={setEditVal} compact/>
                                <div className="flex gap-2">
                                  <button onClick={() => saveEdit(item.id, sub.id)} className="flex-1 bg-[#E0197D] text-white text-xs py-1.5 rounded-lg">Save</button>
                                ...
                                {isManager && (
                                  <div className="w-12 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                    <button onClick={() => { setEditing(item.id); setEditVal({name:item.name,units:item.units||'',details:item.details||''}) }}
                                      className="text-gray-300 hover:text-[#E0197D]"><i className="ti ti-pencil" style={{fontSize:12}}/></button>
                        ...
                        {isManager && (
                          addingTo === sub.id ? (
                            <div className="p-2 border-t border-gray-100 bg-[#FCE4F3] flex flex-col gap-1.5">
                              <input value={newItem.name} onChange={e=>setNewItem(v=>({...v,name:e.target.value}))}
                                placeholder="Item name" dir="ltr"
                                className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg bg-white outline-none focus:border-[#E0197D]"/>
                              ...
                              <div className="flex gap-2">
                                <button onClick={() => addItem(sub.id, sub.param_type)} disabled={saving}
                                  className="flex-1 bg-[#E0197D] text-white text-xs py-1.5 rounded-lg disabled:opacity-50">
                                  {saving ? 'Saving...' : 'Add'}
                                </button>
                            ...
                            <button onClick={() => { setAddingTo(sub.id); setNewItem({name:'',units:'',details:''}) }}
                              className="w-full py-2 text-[12px] text-gray-400 hover:text-[#E0197D] hover:bg-[#FCE4F3] transition-colors border-t border-gray-50 flex items-center justify-center gap-1">
