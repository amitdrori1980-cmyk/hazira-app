...
export default function EquipmentPage() {
  ...
  function ParamFields({ paramType, val, setVal, compact=false }) {
    const cls = `text-sm px-2 py-1.5 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#CC1010] ${compact ? 'text-[12px]' : ''}`
    return (
          ...
          <div key={cat.id} className="mb-3">
            ...
            <button onClick={() => toggleCat(cat)}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all flex-row-reverse text-right ${
                openCat === cat.id
                  ? 'bg-[#CC1010] text-white border-[#CC1010]'
                  : 'bg-white border-gray-100 hover:border-[#CC1010] text-gray-800'
              }`}>
            ...
            {/* Subcategories */}
            {openCat === cat.id && (
              <div className="mt-1 mr-3 border-r-2 border-[#CC1010] pr-3">
                  ...
                  <div key={sub.id} className="mb-2">
                    ...
                    <button onClick={() => toggleSub(sub)}
                      className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg border transition-all flex-row-reverse text-right ${
                        openSub === sub.id
                          ? 'bg-[#FDEAEA] border-[#CC1010] text-[#CC1010]'
                    ...
                    {openSub === sub.id && (
                      <div className="mt-1 bg-white border border-gray-100 rounded-lg overflow-hidden">
                            ...
                            {editing === item.id ? (
                              <div className="p-2 flex flex-col gap-1.5">
                                <input value={editVal.name} onChange={e=>setEditVal(v=>({...v,name:e.target.value}))}
                                  className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#CC1010]" dir="ltr"/>
                                <ParamFields paramType={sub.param_type} val={editVal} setVal={setEditVal} compact/>
                                <div className="flex gap-2">
                                  <button onClick={() => saveEdit(item.id, sub.id)} className="flex-1 bg-[#CC1010] text-white text-xs py-1.5 rounded-lg">Save</button>
                        ...
                        {isManager && (
                          addingTo === sub.id ? (
                            <div className="p-2 border-t border-gray-100 bg-[#FDEAEA] flex flex-col gap-1.5">
                              <input value={newItem.name} onChange={e=>setNewItem(v=>({...v,name:e.target.value}))}
                                placeholder="Item name" dir="ltr"
                                className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg bg-white outline-none focus:border-[#CC1010]"/>
                              ...
                              <div className="flex gap-2">
                                <button onClick={() => addItem(sub.id, sub.param_type)} disabled={saving}
                                  className="flex-1 bg-[#CC1010] text-white text-xs py-1.5 rounded-lg disabled:opacity-50">
                                  {saving ? 'Saving...' : 'Add'}
                                </button>
