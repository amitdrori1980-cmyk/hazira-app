        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-5 w-full max-w-sm shadow-xl">
            ...
            <select value={selectedSpecEvent} onChange={e => setSelectedSpecEvent(e.target.value)}
              className="w-full text-sm px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#FF3EB5] mb-4 text-right">
      ...
      <div className="grid grid-cols-2 gap-3">
        {EXPORTS.map(exp => (
          <button key={exp.id}
            ...
            disabled={!!loading}
            className={`bg-white border rounded-xl p-4 flex flex-col items-center gap-3 transition-all hover:border-[#FF3EB5] hover:shadow-sm disabled:cursor-not-allowed ${
              done === exp.id ? 'border-[#085041] bg-[#E1F5EE]' :
              loading === exp.id ? 'border-[#FF3EB5] bg-[#FFE6F5]' : 'border-gray-100'
            }`}>
            <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${
              done === exp.id ? 'bg-[#E1F5EE]' : 'bg-[#FFE6F5]'
            }`}>
              {loading === exp.id ? (
                <i className="ti ti-loader-2 animate-spin text-[#FF3EB5]" style={{fontSize:22}}/>
              ) : done === exp.id ? (
                <i className="ti ti-circle-check text-[#085041]" style={{fontSize:22}}/>
              ) : (
                <i className={`ti ${exp.icon} text-[#FF3EB5]`} style={{fontSize:22}}/>
              )}
            </div>
            <div className="text-center">
              <div className="text-[13px] font-medium text-gray-800">{exp.label}</div>
              <div className={`text-[11px] mt-0.5 ${done===exp.id?'text-[#085041]':loading===exp.id?'text-[#FF3EB5]':'text-gray-400'}`}>
