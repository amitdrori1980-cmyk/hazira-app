...
export default function EventsPage() {
  ...
  async function addEvent(e) {
    ...
    setAdding(true)
    const payload={...form, depts:form.depts.length?form.depts:depts}
              ...
              {editing===ev.id ? (
                <div className="py-2 flex flex-col gap-2">
                  ...
                  <input value={editVal.description} onChange={e=>setEditVal(v=>({...v,description:e.target.value}))}
                    placeholder="תיאור" className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#CC1010]"/>
                  ...
                  <textarea value={editVal.crew_notes||''} onChange={e=>setEditVal(v=>({...v,crew_notes:e.target.value}))}
                    placeholder="הערות לצוות..." rows={2}
                    className="text-sm px-2 py-1.5 border border-gray-200 rounded-lg bg-gray-50 outline-none focus:border-[#CC1010] resize-none"/>
                  <div className="flex gap-2">
                    <button onClick={()=>saveEdit(ev.id)} className="flex-1 bg-[#CC1010] text-white text-sm py-1.5 rounded-lg">שמור</button>
                          ...
                          {allCrew.map(member=>{
                            ...
                            return (
                              <button key={member.id} onClick={()=>toggleCrewMember(ev.id,member.id)}
                                className={`text-[11px] px-2.5 py-1 rounded-full border transition-colors ${assigned?'bg-[#CC1010] text-white border-[#CC1010]':'border-gray-200 text-gray-600 hover:border-[#CC1010] bg-white'}`}>
                            ...
                            return (
                              <div key={item.id} className="flex items-center gap-2 flex-row-reverse">
                                <button onClick={()=>toggleEquipItem(ev.id,item.id)}
                                  className={`flex-1 text-right text-[12px] px-2.5 py-1.5 rounded-lg border transition-colors ${assigned?'bg-[#FDEAEA] border-[#CC1010] text-[#8B0000] font-medium':'border-gray-200 text-gray-600 hover:border-[#CC1010] bg-white'}`}>
                                  <div>{item.name}</div>
                                ...
                                {assigned&&(
                                  <input
                                    ...
                                    placeholder="כמות"
                                    className="w-16 text-[11px] px-2 py-1.5 border border-gray-200 rounded-lg bg-white outline-none focus:border-[#CC1010] text-center"
                                  />
                                )}
                              </div>
                            )
                            ...
                            {assignedEquip.map(row=>{
                              ...
                              return item?(
                                <div key={row.id} className="flex items-center gap-2 flex-row-reverse text-[11px]">
                                  ...
                                  {row.quantity_needed&&<span className="text-[#CC1010] font-medium">{row.quantity_needed}</span>}
